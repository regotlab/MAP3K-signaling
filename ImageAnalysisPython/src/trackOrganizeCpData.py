...
def linkResultSets(data, handles): 
    ...
    moduleNamesPossible = ['IdentifyPrimaryObjects', 'Watershed','FillObjects','IdentifySecondaryObjects', 'IdentifyTertiaryObjects', 'ExpandOrShrinkObjects', 'ConvertObjectsToImage', 'Morph', 'ApplyThreshold', 'EnhanceOrSuppressFeatures']
    varvalIdxForResult = [1, 1, 1, 1, 2, 1, 1, 1, 1, 1]
    varvalIdxForParent = [0, 0, 0, 3, 0, 0, 0, 0, 0, 0]
...
def trackOrganizeCpData(inputStruct, subfolderNow):
    ...
    objectExists = []
    print(inputStruct['objectSetNames'])
    for i in inputStruct['objectSetNames']:
        if i not in ['Experiment', 'Image']:
            print(i)
            print(handles.Measurements._fieldnames)
    ...
    if nFrames > 1:
        ...
        tracks['particle'] = list(range(0, nObjectsFrame[0]))
    ...
    hdf.savemat(outputFilepath, {'data':data})
    return trackStatus
