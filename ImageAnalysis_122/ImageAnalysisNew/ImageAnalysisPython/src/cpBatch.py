...
def analyzeParentDirCp(pipelineFilepath, inputParentPath, outputParentPath, outputFilename='cpData.mat', subfolderNames=[],
    ...
    workerQueue2 = multiprocessing.Queue()
    writerQueue2 = multiprocessing.Queue()
    
    feedProc = multiprocessing.Process(target=feedFunc, args=(workerQueue, argsList))
    analyzeProc = [multiprocessing.Process(target=analyzeImageDirCpWrap, args=(workerQueue2, writerQueue)) for ii in range(n)]
    writeProc = multiprocessing.Process(target=writeFunc, args=(writerQueue2, logFilepath))
    print(len(argsList))


    feedProc.start()
    for iii in range(len(argsList)):
        workerQueue2.put(workerQueue.get())
    feedProc.join()
    for p in analyzeProc:
        p.start()
    for iiii in range(len(argsList)):
        writerQueue2.put(writerQueue.get())
    for p in analyzeProc:
        p.join()
    writeProc.start()
    writeProc.join()
...
if __name__ == '__main__':
    ...
    cpArgs = {}
    print(sys.argv[1])
    cpArgs['pipelineFilepath'] = sys.argv[1]
    cpArgs['inputParentPath'] = sys.argv[2]
    cpArgs['outputParentPath'] = sys.argv[3]
    cpArgs['outputFilename'] = sys.argv[4]
    cpArgs['subfolderNames'] = sys.argv[5]
    if cpArgs['subfolderNames'] == '[]': cpArgs['subfolderNames'] = []
    cpArgs['frameFirstLast'] = sys.argv[6]
    if cpArgs['frameFirstLast'] == '[]': cpArgs['frameFirstLast'] = []
    cpArgs['cellProfilerPath'] = sys.argv[7]
    cpArgs['preClean'] = sys.argv[8]
    cpArgs['runCellProfiler'] = sys.argv[9]
    cpArgs['runChannelImageCompression'] = sys.argv[10]
    cpArgs['nMaxCores'] = sys.argv[11]
    print(cpArgs)
