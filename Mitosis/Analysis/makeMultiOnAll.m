...
function makeMultiOnAll(totalnum,fpath)

kmax = str2num(totalnum)

str12 = strcat(fpath,'\Pos')

folder = strcat(str12,string(1),'Registration\')
...
for k = 1:kmax
    folder2 = strcat(str12,string(k),'Registration\')
    disp(strcat(folder2,'cpDataTracked.mat'))
...
fid1 = fopen(strcat(folder2,'MitosisClassifier.ts'),'w')
